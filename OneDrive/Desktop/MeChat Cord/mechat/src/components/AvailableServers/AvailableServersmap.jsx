...
import { Dialog, makeStyles, withStyles } from '@mui/material';
...
function AvailableServersmap({server}) {
  ...
  const handleClose = () => {
    ...
    const handlechangeserver=(server)=>{
      navigate.push(`/channels/${server.id}/${channels[0].id}`);
    ...
    const StyledMenuItem = withStyles((theme) => ({
      ...
      const ChangeServer=()=>{
        ...
        //  await channelRef.orderBy('createdAt').limit(1).get().then((snapshot)=>{
              ...
              navigate.push(`/discord-clone/channels/${server.id}/${channels[0].id}`);               
            }
